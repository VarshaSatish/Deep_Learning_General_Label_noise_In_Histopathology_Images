...
import pandas as pd
import myTransforms
from RCE_loss import Robust_CELoss
import warnings
...
if(imagenet_weights):
    folder_name ="./baseline_RCEloss_imagenet/Expt_baseline_{}_{}_ood_{}_lnoise_{}".format(EXPT_NO,OPTIMIZER,ood_noise_val,l_noise_val)
else:
    folder_name ="./baseline_RCEloss_contrastive/Expt_baseline_{}_{}_ood_{}_lnoise_{}".format(EXPT_NO,OPTIMIZER,ood_noise_val,l_noise_val)
...
def parse_option():
    ...
    opt.model_name = 'Expt_{}_{}_{}_lr_{}_decay_{}'.\
...
def set_loader(opt):
    ...
    train_loader = DataLoader(train_dat, batch_size=batch_size, shuffle=True, num_workers=8)

    val_dat = ValDataset(transform = composed_notf)
    val_loader = DataLoader(val_dat, batch_size=batch_size, shuffle=True, num_workers=8)
...
def validation(epoch, model, test_loader, criterion ,args):
    ...
    with torch.no_grad():
        for batch_idx, i in enumerate(test_loader):
            ...
            targets_onehot = torch.nn.functional.one_hot(targets.long(), num_classes=3)
            loss = criterion(output, targets_onehot,_q=0.7)
...
def main():
    ...
    for epoch in range(1, opt.epochs+1):
        ...
        cross_entropy_loss = Robust_CELoss()
        model.train()
        correct = 0
        total = 0
        start = 0
        for idx, sample in enumerate(train_loader,0):
            ...
            labels_onehot = torch.nn.functional.one_hot(labels.long(), num_classes=3)
            _q=0.7
            loss_weighted = cross_entropy_loss(out,labels_onehot,_q).to(device)
            # exit()
        ...
        print('Train: {0}\t' 'RCE loss ={loss.val:.3f}, train accuracy = {1}\t, loss average =({loss.avg:.3f})\t'.format(epoch, train_acc, loss=loss_main))
        sys.stdout.flush()
