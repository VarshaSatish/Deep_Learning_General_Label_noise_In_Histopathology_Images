...
import argparse
from util import get_data, get_model, get_resnet
import os
...
def create_train_pairs(x, y, weighted_y_train, outlier_indices, noise_indices, true_indices):
    ...
    for d in range(3):
        # print('noise_idx',noise_indices[d])
        # print('outlier_idx',outlier_indices[d])
        non_outliers[d] = np.setdiff1d(noise_indices[d], outlier_indices[d])
        # print(non_outliers[d])
    n = min([len(non_outliers[d]) for d in range(3)]) -1
    print(n)
    for d in range(3):
        for i in range(n):
...
def train(dataset='cifar10', batch_size=100, epochs=50, m=1):
    print('Data set: %s' % dataset)

    ood_noiserate = 0.2
    label_noise_rate = 0.1
    if m == 0:
        X_train, Y_train, X_test, Y_test = get_data(dataset, ood_noiserate, label_noise_rate)
    else:
    ...
    total_noise_rate = ood_noiserate+label_noise_rate
    Y_true = Y_train[0:int(Y_train.shape[0] * (1 - total_noise_rate))]
    true_indices = [np.where(Y_true == i)[0] for i in range(0,3)]
    noise_indices = [np.where(Y_train == i)[0] for i in range(0,3)]
    ...
    output1 = Dense(3, activation='softmax', name='output1')(processed_a)
    output2 = Dense(3, activation='softmax', name='output2')(processed_b)
    ...
    y_train_true = np_utils.to_categorical(Y_train, 3)
    ...
    y_test_true = np_utils.to_categorical(Y_test, 3)
...
def get_outlier_factor(X_train_features, y_train_true, class_indices, cumulative_score, m):
    clf = LocalOutlierFactor(n_neighbors=int(2), metric='cosine', contamination=0.4)
    scores = {}
    for i in range(3):
    ...
    outlier = [np.where(cumulative_score[i] <= np.percentile(cumulative_score[i], 50))[0] for i in range(3)]
    #print("Outlier: ",outlier)
    outlier_index = [class_indices[i][outlier[i]] for i in range(3)]
    #print("Outlier index: ",outlier_index)
    weight = [cumulative_score[i][outlier[i]] / (m+1) for i in range(3)]

    for i in range(3):
        for index, j in enumerate(outlier_index[i]):
            y_train_true[j][i] = weight[i][index]
    weighted_y_train = np.max(y_train_true, axis = 1)/3.0
...
if __name__ == "__main__":
    ...
    args = parser.parse_args(['-d', 'cifar10-cifar100', '-e', '10', '-b', '8'])
    main(args)
