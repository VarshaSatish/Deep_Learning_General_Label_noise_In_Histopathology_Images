...
def create_train_pairs(x, y, weighted_y_train, outlier_indices, noise_indices, true_indices):
    ...
    n = min([len(non_outliers[d]) for d in range(4)]) -1
...
def train(dataset='cifar10', batch_size=100, epochs=50, m=0):
    ...
    true_indices = [np.where(Y_true == i)[0] for i in range(0,4)]
    noise_indices = [np.where(Y_train == i)[0] for i in range(0,4)]
    ...
    input_a = Input(shape=(150, 150, 3), name='input_a')
    input_b = Input(shape=(150, 150, 3), name='input_b')
    ...
    output1 = Dense(4, activation='softmax', name='output1')(processed_a)
    output2 = Dense(4, activation='softmax', name='output2')(processed_b)
    ...
    y_train_true = np_utils.to_categorical(Y_train, 4)
    ...
    y_test_true = np_utils.to_categorical(Y_test, 4)
...
def get_outlier_factor(X_train_features, y_train_true, class_indices, cumulative_score, m):
    ...
    for i in range(4):
        ...
        if m == 0:
            ...
            cumulative_score[i] += scores[i]
    outlier = [np.where(cumulative_score[i] <= np.percentile(cumulative_score[i], 50))[0] for i in range(4)]
    outlier_index = [class_indices[i][outlier[i]] for i in range(4)]
    weight = [cumulative_score[i][outlier[i]] / (m+1) for i in range(4)]

    for i in range(4):
        for index, j in enumerate(outlier_index[i]):
            y_train_true[j][i] = weight[i][index]
    weighted_y_train = np.max(y_train_true, axis = 1)/4.0
...
if __name__ == "__main__":
    ...
    args = parser.parse_args(['-d', 'cifar10-cifar100', '-e', '10', '-b', '16'])
    main(args)
